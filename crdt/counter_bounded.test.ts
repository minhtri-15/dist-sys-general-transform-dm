...
import { CounterBoundedCRDT, type CounterBoundedInternalState, CounterBoundedInternalStateSerializer, type CounterBoundedOperation, type ID } from "./counter_bounded";
import { InvalidOperationError } from "./operation.exception";
...
function apply_remote_operation(remote_op: CounterBoundedOperation, state: CounterBoundedInternalState): CounterBoundedInternalState {
  const transformed_local = crdt.downstream(remote_op, state)
  const new_state = crdt.update(transformed_local, state)
  return new_state
}
...
describe("CounterBoundedCRDT", () => {
  ...
  it ("test increment operations.", () => {
    ...
    const id1 = get_id()
    const id2 = get_id()
    const counter1 = apply_remote_operation([id1, 'increment', 10], counter0)
    const counter2 = apply_remote_operation([id2, 'increment', 5], counter1)
  ...
  it ("test decrement operations.", () => {
    ...
    const id1 = get_id()
    const counter1 = apply_remote_operation([id1, 'increment', 10], counter0)
    const counter2 = apply_remote_operation([id1, 'decrement', 6], counter1)

    expect(counter2.decrement.get(id1)).toBe(6)
    expect(crdt.get_global_permissions_to_decrease(counter2)).toBe(4)
    expect(() => apply_remote_operation([id1, 'decrement', 6], counter2), 
    ...
    expect(() => apply_remote_operation([id2, 'decrement', 6], counter2),
  ...
  it ("test a more complex chain of increment and decrement operations.", () => {
    ...
    const counter1 = apply_remote_operation([id1, 'increment', 10], counter0)
    const counter2 = apply_remote_operation([id1, 'decrement', 6], counter1)
    const counter3 = apply_remote_operation([id2, 'increment', 6], counter2)

    expect(crdt.get_global_permissions_to_decrease(counter3), 
      "Test several replicas (balance each other)"
    ).toBe(10)
    expect(() => apply_remote_operation([id1, 'decrement', 6], counter3), 
    "Test forbidden permissions, when total is higher than consumed"
    ).toThrow(InvalidOperationError);

    const counter4 = apply_remote_operation([id2, 'decrement', 6], counter3)
  ...
  it ("test transferring permissions.", () => {
    ...
    const id2 = get_id()
    const counter1 = apply_remote_operation([id1, 'increment', 10], counter0)
    const counter2 = apply_remote_operation([id1, 'transfer', 6, id2], counter1)
    // %% .
    ...
    expect(() => apply_remote_operation([id1, 'transfer', 5, id2], counter2), "Test transference forbidden by lack of previously transfered resources (5 > current 4)"
    ).toThrow(InvalidOperationError)
    //
    "Test transference enabled by previously transfered resources"
    const counter3 = apply_remote_operation([id2, 'transfer', 5, id1], counter2)
  ...
  it("test transfer itself but with not enough permission", () => {
    ...
    const id1 = get_id()
    const counter1 = apply_remote_operation([id1, 'increment', 10], counter0)
    expect(() => apply_remote_operation([id1, 'transfer', 16, id1], counter1))
      .toThrow(InvalidOperationError)
  })
  ...
  it("test transfer itself with enough permission", () => {
    ...
    const id1 = get_id()
    const counter1 = apply_remote_operation([id1, 'increment', 10], counter0)
    const counter2 = apply_remote_operation([id1, 'increment', 5], counter1)
    const counter3 = apply_remote_operation([id1, 'transfer', 5, id1], counter1)
  ...
  it ("test serialization functions `to_binary()' and `from_binary()'.", () => {
    ...
    const counter1 = apply_remote_operation([id1, 'increment', 10], counter0)
    const counter2 = apply_remote_operation([id1, 'increment', 5], counter1)
    const counter3 = apply_remote_operation([id1, 'transfer', 5, id1], counter2)
